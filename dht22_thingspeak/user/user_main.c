 ...
 *  DHT22 2 (DATA_OUT) to ESP Pin GPIO4
 *  DHT22 3 (NC)
 *  DHT22 4 (GND) to GND
 *
...
LOCAL void ICACHE_FLASH_ATTR dht22_cb(void *arg)
{
	...
	static char temp[10];
	static char hum[10];
	static char vdd33[10];
	struct dht_sensor_data* r;
	float lastTemp, lastHum, lastVdd33;
	...
	switch(connState)
	{
		case WIFI_CONNECTED:
			...
			lastHum = r->humidity;
			lastVdd33 = system_get_vdd33() / 1024.;
			if(r->success)
			{
                    ...
                    os_sprintf(vdd33, "%d.%d", (int)(lastVdd33), (int)((lastVdd33 - (int)lastVdd33) * 100));

					os_printf("Temperature: %s *C, Humidity: %s %%, Voltage: %s V\r\n", temp, hum, vdd33);
					// Start the connection process
					os_sprintf(data, "http://%s/update?key=%s&field1=%s&field2=%s&field3=%s&status=dev_ip:%d.%d.%d.%d", THINGSPEAK_SERVER, THINGSPEAK_API_KEY, temp, hum, vdd33, IP2STR(&ipConfig.ip));
					os_printf("Request: %s\r\n", data);
...
void user_init(void)
{
    ...
    float vdd33 = system_get_vdd33() / 1024.;
    os_printf("Vdd33 = %d.%d V\r\n", (int)vdd33, (int)((vdd33 - (int)vdd33) * 100));
